    ...
    def security_access_request_seed(can_p: CanParam, sa_keys, stepno=270,\
        ...
        if ecu_mode in ('DEF', 'EXT'):
            ...
            if sa_keys["SecAcc_Gen"] == 'Gen1':
                logging.info("PBL Gen1 seed send")
                cpay: CanPayload =\
            ...
            #SA_GEN2:
                ...
                #                            +'35CDFBEDEADC3E8F'\
                #                            +'F3B743A7')
                logging.info("PBL Gen2 seed sent")
                cpay: CanPayload =\
                      {"payload" : payload,\
                       "extra" : ''
                      }
            ...
            #SA_GEN2:
            SSA.set_level_key(1)
            logging.info("SBL Gen2 request sent")
            payload = SSA.prepare_client_request_seed()
            cpay: CanPayload =\
                {"payload" : payload,\
                 "extra" : ''
        ...
        if ecu_mode == 'SBL':
            ...
            logging.info("PBL request seed")
            logging.info("CAN_PAYLOAD: %s", cpay)

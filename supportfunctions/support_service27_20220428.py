...
class SupportService27:
    ...
    def security_access_request_seed(can_p: CanParam, sa_keys, stepno=270,# pylint: disable=too-many-branches
        ...
        def __prepare_request_PBL(sa_keys):
            ...
            logging.info("Seed requested from ECU using the following message: %s",payload.hex())
            return cpay
        ...
        if ecu_mode in ('DEF', 'EXT'):
            ...
            logging.info("Return of prepare_request_PBL")
            logging.info("Return cpay %s ", cpay)
            logging.info("bool(cpay) %s ", bool(cpay))
        ...
        if not result:
            logging.info("request seed failed")
            logging.info("reply to teststep: %s ", SC.can_messages[can_p["receive"]][0][2])

...
def run():  # pylint: disable=too-many-statements
    ...
    if (not message.find('F120') == -1) and (not message.find('F12E') == -1):
        logging.info(
            "Step 4: Application Diagnostic Database Part Number found in message."
        )
        logging.info("Step 4: ECU Software Part Numbers found in message.")
        SUTE.pp_combined_did_eda0_becm_mode1_mode3(message,
                                                   "EDA0 for mode1/mode3:\n")
    else:
    ...
    # result:
    logging.info(
        "Step 5: Wait longer than timeout for staying in current mode.")
    ...
    # result:
    logging.info(
        "Step 8: Wait longer than timeout for staying in current mode.")
    logging.info(
        "Step 8: Tester present not sent, no request to ECU as before.")
    time.sleep(8)

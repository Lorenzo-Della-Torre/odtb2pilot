...
import odtb_conf
#ODTB2_DUT = 'bsw-pi11.dhcp.nordic.volvocars.net'
#ODTB2_PORT = '50051'
...
def step_2(can_p: CanParam):
    ...
    """
    cpay: CanPayload = {
        "payload": SC_CARCOM.can_m_send("ReadDataByIdentifier", b'\xF1\x25',
                                        b''),
        "extra": ''
    }
    etp: CanTestExtra = {
    ...
    if not len(SC.can_messages[can_p["receive"]]) == 0:
        ...
        if not pos1 == -1:
            logging.info(
                "Primary Bootloader Software Part Number %s\n",
                SUTE.pp_partnumber(message[pos1 + 4:pos1 + 18],
                                   message[pos1:pos1 + 4]))
        else:
            result = False
    else:
...
def step_3(can_p: CanParam):
    ...
    """
    cpay: CanPayload = {
        "payload": SC_CARCOM.can_m_send("ReadDataByIdentifier", b'\xF1\x26',
                                        b''),
        "extra": ''
    }
    etp: CanTestExtra = {
...
def step_6(can_p: CanParam):
    ...
    """
    cpay: CanPayload = {
        "payload": SC_CARCOM.can_m_send("ReadDataByIdentifier", b'\xF1\x2C',
                                        b''),
        "extra": ''
    }
    etp: CanTestExtra = {
    ...
    if not len(SC.can_messages[can_p["receive"]]) == 0:
        ...
        if not pos1 == -1:
            logging.info(
                "ECU Software Structure Part Number %s\n",
                SUTE.pp_partnumber(message[pos1 + 4:pos1 + 18],
                                   message[pos1:pos1 + 4]))
        else:
            result = False
...
def step_7(can_p: CanParam):
    ...
    """
    cpay: CanPayload = {
        "payload": SC_CARCOM.can_m_send("ReadDataByIdentifier", b'\xF1\x2E',
                                        b''),
        "extra": ''
    }
...
def step_10(can_p: CanParam):
    ...
    """
    cpay: CanPayload = {
        "payload": SC_CARCOM.can_m_send("ReadDataByIdentifier", b'\xF1\x21',
                                        b''),
        "extra": ''
    }
    ...
    if not len(SC.can_messages[can_p["receive"]]) == 0:
        ...
        if not pos1 == -1:
            logging.info(
                "Primary Bootloader Diagnostic Database %s\n",
                SUTE.pp_partnumber(message[pos1 + 4:pos1 + 18],
                                   message[pos1:pos1 + 4]))
        else:
            result = False
...
def run():
    ...
    can_p: CanParam = {
        'netstub':
        SC.connect_to_signalbroker(odtb_conf.ODTB2_DUT, odtb_conf.ODTB2_PORT),
        #SC.connect_to_signalbroker(ODTB2_DUT, ODTB2_PORT),
        'send':
        "Vcu1ToBecmFront1DiagReqFrame",
        'receive':
        "BecmToVcu1Front1DiagResFrame",
        'namespace':
        SC.nspace_lookup("Front1CANCfg0")
    }
    ...
    logging.info("Time needed for testrun (seconds): %s",

...
def step_1(can_p):
    ...
    did_pressure_new = SIO.extract_parameter_yml(str(inspect.stack()[0][3]),
                                                 'did_pressure')
    ...
    logging.info("Step%s: did_pressure after YML: %s", step_no,
                 did_pressure.hex())

    result, pressure = SE22.read_did_pressure_sensor(can_p, did_pressure, b'',
                                                     '1')
    logging.info("Pressure returned: %s \n", pressure)
    logging.info("Step%s, received frames: %s", step_no,
                 SC.can_frames[can_p["receive"]])
    logging.info("Step%s, received messages: %s", step_no,

...
# project:  BECM/HVBM CMS emulator
...
def step_1(can_p):
    """
        teststep 1: request DID CMS FD61 (MEP2 HVBM CMS Communication Status)
    """
    ...
    #here: if reply should be a posivite reply, so it contains 62FD61
    if SUTE.test_message(SC.can_messages[can_p['receive']], '62FD61'):
        logging.info("Reply contains 62FD61 as expected.")

    if SC.can_messages[can_p['receive']][0][2][0:1] == '0':
    # show single frame reply
        length_pl = SC.can_messages[can_p['receive']][0][2][1:2]
        mess_rec = SC.can_messages[can_p['receive']][0][2][2::]
        logging.info("Length reply: %s (0x%s)", int(length_pl, 16), length_pl)
        logging.info("payload received: %s", mess_rec)
    elif SC.can_messages[can_p['receive']][0][2][0] == '1':
    #This was intended for MF replies
        first_frame= int(SC.can_messages[can_p['receive']][0][2][1:4], 16)
        mess_rec = SC.can_messages[can_p['receive']][0][2][4::]
        logging.info("Length reply: %s (0x%s)", first_frame,
                     SC.can_messages[can_p['receive']][0][2][1:4])
        #logging.info("DID sent:                    %s",
        #             SC.can_messages[can_p['receive']][0][2][4:10])
        logging.info("payload received: %s", mess_rec)
    #ret_str = 'PL as bin:'
    #for i in first_frame-4:
    #    ret_str = ret_str + ' {0:04b}'.format(int(a[0],16))
    #logging.info("%s", ret_str)

    #logging.info("CMS current (HvBatt/ActRaw): %s", SC.can_messages[can_p['receive']][0][2][10:14])
    #logging.info("CoulombCnt:                  %s", SC.can_messages[can_p['receive']][0][2][14:18])
    #logging.info("TempNTC (HvBatt/ActNtc):     %s", SC.can_messages[can_p['receive']][0][2][18:22])
...
def step_2(can_p):
    """
        teststep 4: request DID CMS FD40 (MEP2 HVBM CMS Error)
    """
    ...
    #if you want to test if received message contains a certain string:
    #here: if reply should be a posivite reply, so it contains 62FD40
    if SUTE.test_message(SC.can_messages[can_p['receive']], '62FD40'):
        logging.info("Reply contains 62FD40 as expected.")
...
def run():
    ...
    #for i in range(2):
        # step1:
        # action: Request FD61 (CMS communication status)
        # result: Show status CMS communication
        result = step_1(can_p) and result

        # step2:
        # action: Request FD40 (CMS Error)
        # result: Show SM in readable form

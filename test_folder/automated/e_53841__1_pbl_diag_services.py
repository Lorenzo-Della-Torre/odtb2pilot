...
def step_2(dut: Dut):
    ...
    expected_result: PBL DIDs extracted from sddb file
    """
...
def step_3(dut, pbl_did_dict):
    ...
    expected_result: True when received positive response for all DIDs
    """
    ...
    if not result:
        logging.error("Test Failed: Failed to receive positive response for all DIDs")
        return False
    logging.info("Successfully received positive response for all DIDs")
    return True
...
def run():
    """
    Reading DIDs from sddb file in programming session and validate response with
    service 0x22
    """
    dut = Dut()

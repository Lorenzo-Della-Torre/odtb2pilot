...
def step_2(dut: Dut):
    """
    action: Software download request of SBL and ESS VBFs file type. Verify which part
            of software download is failed.

    expected_result: True when Complete or Compatible bit & EXE bit set to 1
    """
    # Set default session
    dut.uds.set_mode(1)
    time.sleep(10)
    ...
    pos = message.find('0205')
    value = "{:040b}".format(int(message[pos+6:pos+16], 16))
    # Verify Complete and EXE bit set to 1
    if value[39] == '1' and value[37] == '1':
        logging.info("Complete bit (%s) and EXE bit (%s) received" , value[39], value[37])
        return True

    logging.error("Test failed: Expected Complete and EXE bit 1, received Complete bit"
                     " (%s) and EXE bit (%s)", value[39], value[37])
    return False


def step_3(dut: Dut):
    """
    action: Software download request for SBL and empty ESS VBFs file types. Verify ESS
            download has failed.

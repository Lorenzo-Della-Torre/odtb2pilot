...
def step_6(can_p):
    """
    Teststep 6: request 10 DID in one request - those with shortest reply
    """
    cpay: CanPayload = {
        "payload": SC_CARCOM.can_m_send("ReadDataByIdentifier",
...
def step_7(can_p, p4_server_max, jitter_testenv):
    """
...
def run():
    ...
    # step 6:
    # action: send ReadDataByIdentifier signal (10 DID) in extended mode
    # result: BECM sends positive reply
    ...
    # action: Verify (time receive message – time sending request) < P4_server_max
    # result: positive result
        result = result and step_7(can_p, p4_server_max, jitter_testenv)

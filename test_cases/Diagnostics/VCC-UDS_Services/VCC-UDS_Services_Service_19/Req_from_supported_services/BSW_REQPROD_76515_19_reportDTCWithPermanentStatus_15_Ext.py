...
def step_2(can_par):
    """
    Teststep 2: verify that ReportDTCWithPermanentStatus reply positively
    """
    stepno = 2
    cpay: CanPayload = SIO.extract_parameter_yml(
        "step_{}".format(stepno),
        payload=SC_CARCOM.can_m_send("ReadDTCInfoReportDTCWithPermanentStatus", b'', b''),
        extra=''
        )
    etp: CanTestExtra = SIO.extract_parameter_yml(
        "step_{}".format(stepno),
        step_no=2,
        purpose="verify that ReportDTCWithPermanentStatus reply positively",
        timeout=5,
        min_no_messages=-1,
        max_no_messages=-1
        )
...
def run():
    ...
    # result: BECM reports extended session
        result = result and SE22.read_did_f186(can_par, dsession=b'\x03')#, 3)

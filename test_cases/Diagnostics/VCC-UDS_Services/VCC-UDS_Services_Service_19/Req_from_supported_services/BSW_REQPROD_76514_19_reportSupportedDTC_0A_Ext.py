...
def step_2(can_par):
    """
    Teststep 2: verify that ReportSupportedDTC reply positively
    """
    stepno = 2
    cpay: CanPayload = SIO.extract_parameter_yml(
        "step_{}".format(stepno),
        payload=SC_CARCOM.can_m_send("ReadDTCInfoReportSupportedDTC", b'', b''),
        extra=''
        )
    etp: CanTestExtra = SIO.extract_parameter_yml(
        "step_{}".format(stepno),
        step_no=2,
        purpose="verify that ReportSupportedDTC reply positively",
        timeout=5,
        min_no_messages=-1,
        max_no_messages=-1
        )

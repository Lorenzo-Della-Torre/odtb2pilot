...
def step_2(can_p, ecu_message):
    """
    teststep 2: Verify that message contains information of DTC for specific Snapshot
    ...
    etp: CanTestExtra = {
        "step_no": 2,
        "purpose": "Verify that message contains information of DTC for specific Snapshot \
            record Number selected from service 19 subservice ReadDTCInfoSnapshotIdentification",
        "timeout": 1,
        "min_no_messages": -1,
        "max_no_messages": -1
...
def run():
    ...
    if result:
        ...
        step_result, ecu_message = step_1(can_p)
        ...
        # record Number selected from service 19 subservice ReadDTCInfoSnapshotIdentification
        # result: BECM reply positively
        step_result, dtc_bytes = step_2(can_p, ecu_message)
        result = result and step_result

        # step 3:
        # action: Verify empty reply using service AF with suppressPosRspMsgIndicationBit = True
        # result: Empty reply
        result = result and step_3(can_p, dtc_bytes)

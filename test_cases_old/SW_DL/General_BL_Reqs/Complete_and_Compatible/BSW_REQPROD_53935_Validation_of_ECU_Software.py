...
def step_3(stub, can_send, can_receive, can_namespace):
    ...
    result = SSBL.check_complete_compatible_routine(stub,\
                                                    can_send, can_receive, can_namespace,\
                                                    stepno, purpose)

    result = result and\
             (SSBL.pp_decode_routine_complete_compatible(SC.can_messages[can_receive][0][2])
...
def step_5(stub, can_send, can_receive, can_namespace):
    ...
    extra_param = {"purpose" : "Verify Programming session",\
                   "timeout" : 1,\
                   "min_no_messages" : 1,\
                   "max_no_messages" : 1
                  }
...
def step_7(stub, can_send, can_receive, can_namespace):
    """
    Teststep 7: Download Compatibles SW Parts
    """
    stepno = 7
...
def step_8(stub, can_send, can_receive, can_namespace):
    ...
    result = SSBL.check_complete_compatible_routine(stub, can_send, can_receive, can_namespace,\
                                                    stepno, purpose)
    result = result and\
             (SSBL.pp_decode_routine_complete_compatible(SC.can_messages[can_receive][0][2])
              == 'Complete, Compatible')
...
def run():
    ...
    result = step_1(network_stub, can_send, can_receive, can_namespace) and result

    # step 2:
    # action:
    # result: BECM sends positive reply
    result = step_2(network_stub, can_send, can_receive, can_namespace) and result

    # step 3:
    # action:
    # result: BECM sends positive reply
    result = step_3(network_stub, can_send, can_receive, can_namespace) and result

    # step 4:
    # action:
    # result: BECM sends positive reply
    result = step_4(network_stub) and result

    # step 5:
    # action:
    # result: BECM sends positive reply
    result = step_5(network_stub, can_send, can_receive, can_namespace) and result


    # step 6:
    # action:
    # result: BECM sends positive reply
    result = step_6(network_stub, can_send, can_receive, can_namespace) and result

    # step 7:
    # action:
    # result: BECM sends positive reply
    result = step_7(network_stub, can_send, can_receive, can_namespace) and result

    # step 8:
    # action:
    # result: BECM sends positive reply
    result = step_8(network_stub, can_send, can_receive, can_namespace) and result

    # step 9:
    # action:
    # result: BECM sends positive reply
    result = step_9(network_stub, can_send, can_receive, can_namespace) and result

    # step 10:
    # action:
    # result: BECM sends positive reply
    result = step_10(network_stub, can_send, can_receive, can_namespace) and result

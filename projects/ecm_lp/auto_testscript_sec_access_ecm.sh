...
### Generate catalog for logfiles and list of scripts to run
    TESTRUN=$(date +Testrun_%Y%m%d_%H%M_sec_access_ECM)
    [ ! -d $TESTRUN ] && mkdir $TESTRUN
    echo "Results of testrun $TESTRUN:" >$TESTRUN\/Result.txt

	# SWRS Testing
	python3 $TESTREPO/autotest/testcase_picker.py --txtfile $TESTREPO/test_folder/automated/sec_access_test.txt --scriptfolder $TESTREPO/test_folder/automated > testscripts_auto.lst

...
def step_3():
    ...
    jitter_testenv = 10
    if (P4_server_max + jitter_testenv) > (T2 - T1):
        testresult 
    else:
        testresult = False
...
def step_4(stub, s, r, ns):
    ...
    testresult = testresult and SuTe.teststep(stub, can_m_send, can_mr_extra, s, r, ns, stepno, purpose, timeout, min_no_messages, max_no_messages)
    T2 = SC.can_messages[r][0][0]
    testresult = testresult and SuTe.PP_Decode_Routine_Control_response(SC.can_frames[r][0][2], "Type2,Completed")

# teststep 5: Verify (time receive message – time sending request) less than P4_server_max
def step_5():
    global testresult
    global T1, T2
    global P4_server_max

...
# teststep 1: verify that SnapshotIdentification are sent
...
# teststep 2: verify that SnapshotRecordByDTCNumber are sent for specific number
def step_2(stub, s, r, ns):
    ...
    stepno = 2
    purpose = "verify that SnapshotRecordByDTCNumber are sent for specific number"
    timeout = 1 #wait a second for reply to be send
    min_no_messages = -1
    max_no_messages = -1

    can_m_send = SC.can_m_send( "ReadDTCInfoSnapshotRecordByDTCNumber", DTCnumber ,Snapshot_RecordNumber)
    can_mr_extra = ''
  
    testresult = testresult and SuTe.teststep(stub, can_m_send, can_mr_extra, s, r, ns, stepno, purpose, timeout, min_no_messages, max_no_messages)
    testresult = testresult and SuTe.test_message(SC.can_messages[r], '5904')
    time.sleep(1)
...
def step_3(stub, s, r, ns):
    ...
    stepno = 3
    purpose = "verify that SnapshotRecordByDTCNumber reply with empty message"
    timeout = 1 #wait a second for reply to be send
    min_no_messages = -1
    max_no_messages = -1

    can_m_send = SC.can_m_send( "ReadDTCInfoSnapshotRecordByDTCNumber(84)", DTCnumber ,Snapshot_RecordNumber)
    can_mr_extra = ''
...
def run():
    ...
    # step 1:
    # action: send signal ReadDTCSnapshotRecordByDTCNumber
    # result: BECM respond positively
    step_2(network_stub, can_send, can_receive, can_namespace)

    # step2:
    # action: send signal ReadDTCSnapshotRecordByDTCNumber (84)

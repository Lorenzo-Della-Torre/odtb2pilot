...
# teststep 2: verify that Read DTC by Status mask failed are sent
def step_2(stub, s, r, ns):
    global testresult
    
    stepno = 2
    purpose = "verify that Read DTC by Status mask failed are sent"
    timeout = 1 #wait a second for reply to be send
    min_no_messages = -1
    max_no_messages = -1

    can_m_send = SC.can_m_send( "ReadDTCByStatusMask", b'\x06\x03\x00' ,"testFailed")
    can_mr_extra = ''
  
    testresult = testresult and SuTe.teststep(stub, can_m_send, can_mr_extra, s, r, ns, stepno, purpose, timeout, min_no_messages, max_no_messages)
    testresult = testresult and SuTe.test_message(SC.can_messages[r], '5902')
    time.sleep(1)

# teststep 3: verify that Read DTC by Status Mask pending is sent
def step_3(stub, s, r, ns):
    ...
    can_m_send = SC.can_m_send( "ReadDTCByStatusMask", b'\x06\x03\x00' ,"pendingDTC")
    can_mr_extra = ''
...
def run():
    ...
    # step2:
    # action: send signal ReadDTCByStatusMask failed
    # result: BECM respond positively
    step_2(network_stub, can_send, can_receive, can_namespace)

    # step3:
    # action: send signal ReadDTCByStatusMask pending

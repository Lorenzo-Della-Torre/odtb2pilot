...
# teststep 1: verify that Read DTC Extent Info are sent
...
# teststep 2: verify that Read DTC Extent Info reply with empty frame
def step_2(stub, s, r, ns):
    global testresult
    
    stepno = 2
    
    purpose = "verify that Read DTC Extent Info reply with empty frame"
    timeout = 1 #wait a second for reply to be send
    min_no_messages = -1
    max_no_messages = -1

    can_m_send = SC.can_m_send( "ReadDTCInfoExtDataRecordByDTCNumber(86)", b'\x0B\x4A\x00' , b'\xFF')
    can_mr_extra = ''
...
def run():
    ...
    ############################################
    # step 1:
    # action: send signal Read DTC Extent Info
    # result: BECM respond positively
    step_1(network_stub, can_send, can_receive, can_namespace)
    
    # step2:
    # action: send signal Read DTC Extent Info (86)

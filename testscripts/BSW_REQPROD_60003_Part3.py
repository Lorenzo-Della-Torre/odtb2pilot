...
def precondition(stub):
    ...
    timeout = 300   #seconds
    #timeout = 40   #seconds
    subscribe_signal(stub, can_send, can_rec, can_nspace, timeout)
    #record signal we send as well
    subscribe_signal(stub, can_rec, can_send, can_nspace, timeout)
...
def step_0(stub):
    ...
    teststep(stub, can_m_send, can_mr_extra, can_send, can_receive, can_namespace, stepno, purpose, timeout, min_no_messages, max_no_messages)
...
def step_3(stub):
    ...
    for count in range(max_delay):
        time.sleep(FC_delay/1000)
        send_FC_frame(stub,can_send,can_namespace,FC_flag,BS,ST)
        #FC = threading.Timer(FC_delay/1000, send_FC_frame, (stub,can_send,can_namespace,FC_flag,BS,ST)).start()
        FC_responses += 1
        print ("DelayNo.: ", FC_responses-1, ", Number of can_frames received: ", len(can_frames[can_receive]))
...
#can_m_send = b'\x10\x02'
    ...
    time.sleep(1)
    update_can_messages(stub, can_receive)
    ...
    test_message(can_messages[can_receive], teststring='62EDA0')
    test_message(can_messages[can_receive], teststring='F120')
    test_message(can_messages[can_receive], teststring='F12A')
    test_message(can_messages[can_receive], teststring='F12B')
    test_message(can_messages[can_receive], teststring='F12E')
    test_message(can_messages[can_receive], teststring='F18C')
